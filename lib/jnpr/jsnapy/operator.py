...
# given for comparision, then it will take first value
    ...
    def no_diff_in(self, x_path, ele_list, err_mssg,
                info_mssg, teston, iter, id_list, xml1, xml2):
        self.print_testmssg("no-diff-in")
        res = True
        iddict = {}
        predict = {}
        postdict = {}
        ...
        if re.match(ele_list[0], "no node"):
            ...
            if (not pre_nodes) or (not post_nodes):
                self.logger_testop.error(colorama.Fore.YELLOW +
                                         "WARNING!! Nodes are not present in given Xpath: <{}>".format(
                                             x_path),
                                         extra=self.log_detail)
                # res = False
                # count_fail = count_fail + 1
